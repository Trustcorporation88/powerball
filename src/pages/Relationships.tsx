...
import { ParsedSheet, ParsedExcel } from "@/utils/excelParser";
import {
  ArrowLeft, ArrowRight, Table2, Link, Trash2, Plus,
  Rows3, Columns3, FileSpreadsheet, GitBranch, X
} from "lucide-react";
...
export default function Relationships() {
  ...
  const removeTable = (id: string) => {
    ...
    setRelationships((prev) => prev.filter((r) => r.leftTableId !== id && r.rightTableId !== id));
  ...
  const tableMap = useMemo(() => {
    const m: Record<string, ImportedTable> = {};
  ...
  const cardPositions = useMemo(() => {
    ...
    tables.forEach((t, i) => {
      ...
      positions[t.id] = { x: 40 + col * 280, y: 60 + row * 180, w: 240, h: 130 };
    });
    return positions;
  }, [tables]);
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Defina como as tabelas se relacionam para análises cruzadas
          </p>
        </div>
          ...
          <Card className="border-slate-200">
            <CardContent className="p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                {tables.length > 1 && (
                  <span className="text-xs text-slate-400">{tables.length} tabelas importadas</span>
                )}
              </div>
                ...
                <div className="relative w-full overflow-x-auto">
                  <svg width={Math.max(900, tables.length * 290)} height={Math.max(300, Math.ceil(tables.length / 3) * 190)} className="mx-auto">
                      ...
                      return (
                        <g key={t.id}>
                          <rect
                            ...
                            height={pos.h}
                            rx={8}
                            fill={t.id === "primary" ? "#ecfdf5" : "#f8fafc"}
                            stroke={t.id === "primary" ? "#10b981" : "#cbd5e1"}
                            strokeWidth={1.5}
                          />
                          <text x={pos.x + 12} y={pos.y + 26} fontSize={12} fontWeight={600} fill="#0f172a">
                            {t.name.length > 22 ? t.name.slice(0, 22) + "…" : t.name}
                          </text>
                          <text x={pos.x + 12} y={pos.y + 48} fontSize={10} fill="#64748b">
                            {t.rowCount} linhas · {t.colCount} colunas
                          </text>
                          <text x={pos.x + 12} y={pos.y + 66} fontSize={9} fill="#94a3b8">
                            {t.fileName.length > 28 ? t.fileName.slice(0, 28) + "…" : t.fileName}
                          </text>
                          {t.id === "primary" && (
                            <>
                    ...
                    {relationships.map((rel) => {
                      const left = cardPositions[rel.leftTableId];
                      const right = cardPositions[rel.rightTableId];
                      ...
                      return (
                        <g key={rel.id}>
                          <path
                            ...
                            fill="none"
                            stroke={rel.joinType === "inner" ? "#8b5cf6" : "#3b82f6"}
                            strokeWidth={1.5}
                            strokeDasharray={rel.joinType === "left" ? "6 3" : "none"}
                          />
                          <circle cx={x1} cy={y1} r={3} fill={rel.joinType === "inner" ? "#8b5cf6" : "#3b82f6"} />
                          <circle cx={x2} cy={y2} r={3} fill={rel.joinType === "inner" ? "#8b5cf6" : "#3b82f6"} />
                          <rect x={midX - 40} y={(y1 + y2) / 2 - 12} width={80} height={18} rx={4} fill="white" stroke={rel.joinType === "inner" ? "#8b5cf6" : "#3b82f6"} strokeWidth={0.5} />
                          <text x={midX} y={(y1 + y2) / 2} textAnchor="middle" fontSize={8} fill={rel.joinType === "inner" ? "#8b5cf6" : "#3b82f6"}>
                            {JOIN_LABELS[rel.joinType]}
                          </text>
                          <text x={(x1 + midX) / 2} y={y1 - 6} textAnchor="middle" fontSize={8} fill="#64748b">
                            {rel.leftColumn}
                          </text>
                          <text x={(x2 + midX) / 2} y={y2 - 6} textAnchor="middle" fontSize={8} fill="#64748b">
                            {rel.rightColumn}
                          </text>
                        </g>
                      );
            ...
            <CardContent className="p-5">
                    ...
                    <Card className="border-emerald-300 bg-emerald-50/50">
                      <CardContent className="p-3 space-y-3">
                        ...
                        <div>
                          ...
                          <Select value={contactForm.leftTableId} onValueChange={(v) => setContactForm({ ...contactForm, leftTableId: v })}>
                        ...
                        <div>
                          ...
                          <Select value={contactForm.leftColumn} onValueChange={(v) => setContactForm({ ...contactForm, leftColumn: v })}>
                            <SelectTrigger className="h-7 text-xs mt-0.5">
                              <SelectValue placeholder="Selecionar coluna" />
                            </SelectTrigger>
                            <SelectContent>
                              {(tableMap[contactForm.leftTableId]?.headers || []).map((h) => (
                        ...
                        <div>
                          ...
                          <Select value={contactForm.rightTableId} onValueChange={(v) => setContactForm({ ...contactForm, rightTableId: v })}>
                            <SelectTrigger className="h-7 text-xs mt-0.5">
                              <SelectValue placeholder="Selecionar tabela" />
                            </SelectTrigger>
                            <SelectContent>
                        ...
                        <div>
                          ...
                          <Select value={contactForm.rightColumn} onValueChange={(v) => setContactForm({ ...contactForm, rightColumn: v })}>
                            <SelectTrigger className="h-7 text-xs mt-0.5">
                              <SelectValue placeholder="Selecionar coluna" />
                            </SelectTrigger>
                            <SelectContent>
                              {(tableMap[contactForm.rightTableId]?.headers || []).map((h) => (
                                <SelectItem key={h} value={h} className="text-xs">{h}</SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        ...
                        <div>
                          ...
                          <Select value={contactForm.joinType} onValueChange={(v: "inner" | "left") => setContactForm({ ...contactForm, joinType: v })}>
                            <SelectTrigger className="h-7 text-xs mt-0.5">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                        ...
                        <Button onClick={addRelationship} size="sm" className="w-full bg-emerald-600 hover:bg-emerald-700 h-7 text-xs">
              ...
              {relationships.length === 0 ? (
                <p className="text-xs text-slate-400 text-center py-6">
                  {tables.length < 2
                    ? "Importe mais tabelas para criar relacionamentos"
                    : "Nenhum relacionamento definido"}
                </p>
              ) : (
                <div className="space-y-2">
                  {relationships.map((rel) => {
                    const lt = getTableById(rel.leftTableId);
                    const rt = getTableById(rel.rightTableId);
                    return (
                      <motion.div
                        key={rel.id}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1 text-xs">
                            <span className="font-medium text-slate-700 truncate">
                              {lt?.name || "?"}.{rel.leftColumn}
                            </span>
                            <span className="text-[10px] px-1 py-0.5 rounded font-medium bg-violet-100 text-violet-700">
                              {JOIN_LABELS[rel.joinType]}
                            </span>
                            <span className="font-medium text-slate-700 truncate">
                              {rt?.name || "?"}.{rel.rightColumn}
                            </span>
                          </div>
                        </div>
                        <button
