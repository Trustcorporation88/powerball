            ...
            <thead className="bg-slate-50 border-b">
              <tr>
                {sheet.headers.map((col) => (
                  <th key={col} className="px-3 py-2.5 text-left font-medium text-slate-700 whitespace-nowrap text-xs">
                    {col}
                  </th>
                ))}
              </tr>
              ...
              {previewData.map((row, i) => (
                <tr key={i} className="border-b last:border-0 hover:bg-slate-50">
                  {sheet.headers.map((header, j) => (
                    <td key={j} className="px-3 py-2 text-slate-600 whitespace-nowrap max-w-[180px] truncate text-xs">
