...
async function getOwnedProject(request, reply, projectId) {
...
export async function projectRoutes(app) {
    ...
    app.delete("/projects/:id", async (request, reply) => {
        if (!(await getOwnedProject(request, reply, request.params.id)))
            return;
        await prisma.project.delete({ where: { id: request.params.id } });
        return reply.send({ success: true });
    });
    app.get("/projects/:id/file", async (request, reply) => {
        if (!(await getOwnedProject(request, reply, request.params.id)))
            return;
    ...
    app.put("/projects/:id/file", async (request, reply) => {
        if (!(await getOwnedProject(request, reply, request.params.id)))
            return;
        ...
        const payload = {
            ...
            selectedSheets: (data.selectedSheets ?? Prisma.JsonNull),
    ...
    app.put("/projects/:id/mappings", async (request, reply) => {
        if (!(await getOwnedProject(request, reply, request.params.id)))
            return;
    ...
    app.get("/projects/:id/transactions", async (request, reply) => {
        if (!(await getOwnedProject(request, reply, request.params.id)))
            return;
        const transactions = await prisma.transaction.findMany({ where: { projectId: request.params.id } });
        return reply.send(transactions.map((t) => ({
            id: t.id,
            date: t.date,
    ...
    app.put("/projects/:id/transactions", async (request, reply) => {
        if (!(await getOwnedProject(request, reply, request.params.id)))
            return;
    ...
    app.get("/projects/:id/dre-rules", async (request, reply) => {
        if (!(await getOwnedProject(request, reply, request.params.id)))
            return;
    ...
    app.put("/projects/:id/dre-rules", async (request, reply) => {
        if (!(await getOwnedProject(request, reply, request.params.id)))
            return;
    ...
    app.get("/projects/:id/layout", async (request, reply) => {
        if (!(await getOwnedProject(request, reply, request.params.id)))
            return;
    ...
    app.put("/projects/:id/layout", async (request, reply) => {
        if (!(await getOwnedProject(request, reply, request.params.id)))
            return;
